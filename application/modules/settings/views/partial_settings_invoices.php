	...
	<div class="control-group">
		...
		<div class="controls">
			<select name="settings[default_include_item_tax]">
				<option value=""><?php echo lang('none'); ?></option>
				<option value="0" <?php if ($this->mdl_settings->setting('default_include_item_tax') === '0') { ?>selected="selected"<?php } ?>>Apply Before Item Tax</option>
				<option value="1" <?php if ($this->mdl_settings->setting('default_include_item_tax') === '1') { ?>selected="selected"<?php } ?>>Apply After Item Tax</option>
			</select>
		</div>
	</div>
